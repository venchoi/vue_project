  ...
  export default {
    ...
    methods: {
      ...
      getMsgCode() {
        ...
        if (self.binding) {
          const phone = self.binding.phone;
          const imgCode = self.binding.code;
          if (self.check('verify', imgCode) && self.check('phone', phone)) {
            if (self.canMsgCode) {
              let countNum = 60;
              let timer = null;
              ...
              http.api[httpList.GET_MSG_CODE]({
                success(data) {
                  self.$store.dispatch(actions.UPDATE_NOTICE_DATA, {
                    status: 'success',
                    content: data.msg,
                  });
                },
                fail(data) {
                  ...
                  self.msgContent = '重新获取';
                  self.canMsgCode = true;
                  if (data.msg === '此帐号没有权限登录，请联系客服开通帐号!') {
                    self.$emit('toShow');
                    return;
                  }
                  self.$store.dispatch(actions.UPDATE_NOTICE_DATA, {
