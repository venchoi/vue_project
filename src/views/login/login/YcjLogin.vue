<template>
  <div class="contents" v-show="isShow">
    <ycj-form :postData="formData" :api="loginApi">
      <form-input name="account" placeholder="手机号码/邮箱" :binding="formData" :reg="reg.phone" icon="mobile"></form-input>
      <div class="pwd">
        <div class="pwd-inner">
          <form-input name="password" placeholder="请输入密码" :binding="formData" :reg="reg.ycjPassword" icon="lock" :inputType="inputType"></form-input>
        </div>
     ...
     <auto-login :binding="autoData"></auto-login>
      ...
      <div class="footer" @click="changeTab($event)"><span>注册用户</span></div>
    </ycj-form>
  </div>
</template>
<script>
